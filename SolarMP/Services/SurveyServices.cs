...
namespace SolarMP.Services
{
    public class SurveyServices : ISurvey
    {
        ...
        public async Task<List<Survey>> GetSurveyById(string? surveyId)
        {
            try
            {
                var data = await this.context.Survey.Where(x => x.Status && x.SurveyId.Equals(surveyId)).ToListAsync();
        ...
        public async Task<bool> InsertSurvey(SurveyDTO survey)
        {
            try
            {
                ...
                _survey.SurveyId = "SUR" + Guid.NewGuid().ToString().Substring(0, 7);
        ...
        public async Task<bool> UpdateSurvey(SurveyDTO upSurvey)
        {
            try
            {
                ...
                if (_survey != null)
                {
                    _survey.Description = upSurvey.Description;
                    _survey.Note = upSurvey.Note;
                    _survey.StaffId = upSurvey.StaffId;
                    _survey.Status = upSurvey.Status;
